...
void Texture2DArrayObject::SetImage<std::byte>(GLint level, GLsizei width, GLsizei height, GLsizei depth, Format format, InternalFormat internalFormat, std::span<const std::byte> data, Data::Type type)
{
    ...
    // https://learnopengl.com/Guest-Articles/2021/CSM adds 1 to depth, maybe to avoid going out of bounds?
    glTexImage3D(GetTarget(), level, internalFormat, width, height, depth + 1, 0, format, static_cast<GLenum>(type), data.data());
}
